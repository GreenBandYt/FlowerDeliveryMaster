...
async def link(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if user.telegram_id and user.telegram_id != telegram_id:
            await update.message.reply_text("Этот логин уже привязан к другому Telegram аккаунту.")
            return
        user.telegram_id = telegram_id
        await sync_to_async(user.save)()
        await update.message.reply_text(f"Ваш Telegram аккаунт успешно привязан к логину {username}.")
    except CustomUser.DoesNotExist:
...
async def orders(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text("Для просмотра деталей введите номер заказа:")
